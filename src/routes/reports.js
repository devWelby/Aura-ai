...
router.post(['/processar_upload.php', '/relatorios/processar-upload'], uploadLimiter, upload.single('extrato'), validateCsrfPost, async (req, res) => {
  try {
    ...
    return res.status(500).send('Erro interno ao processar o arquivo enviado.');
  }
});

router.get(['/ver_relatorio.php', '/relatorios/ver'], readLimiter, async (req, res) => {
  try {
    const isLogged = Boolean(req.session.usuario_id);
    ...
    return res.status(500).send('Erro ao carregar relatorio.');
  }
});

router.get(['/historico.php', '/relatorios/historico'], readLimiter, requireAuth, async (req, res) => {
  try {
    ...
    return res.status(500).send('Erro ao carregar historico.');
  }
});
