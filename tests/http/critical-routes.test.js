...
test('GET /auth/login retorna 200 e inclui csrf_token', async () => {
...
test('GET /auth/cadastro retorna 200 e inclui csrf_token', async () => {
  const res = await request(app).get('/auth/cadastro');
  assert.equal(res.status, 200);
  assert.match(res.text, /name="csrf_token"/i);
});
...
test('GET /pagamentos/planos retorna 200', async () => {
  const res = await request(app).get('/pagamentos/planos');
  assert.equal(res.status, 200);
  assert.match(res.text, /Planos e Precos/i);
});
...
test('GET /relatorios/historico sem auth redireciona para login', async () => {
  const res = await request(app).get('/relatorios/historico');
  assert.equal(res.status, 302);
  assert.equal(res.headers.location, '/login.php');
});
...
test('POST /relatorios/processar-upload com .xls retorna 400', async () => {
  const agent = request.agent(app);
  ...
  const upload = await agent
    .post('/relatorios/processar-upload')
    .field('csrf_token', csrfMatch[1])
    .attach('extrato', Buffer.from('arquivo-legado', 'utf-8'), {
      filename: 'extrato_antigo.xls',
      contentType: 'application/vnd.ms-excel',
